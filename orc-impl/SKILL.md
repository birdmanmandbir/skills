...
The only allowed early Implementation report is a genuine blocker that cannot be resolved without user or Owner judgement. A blocker Implementation report must state the evidence checked, the exact decision required, and the Implementation worker's recommendation.
...
When all tickets and final relevant checks are complete, the Implementation worker writes an Implementation report at a location it chooses. The Implementation report records completed acceptance criteria, commits, changed paths, test results, changed-LOC variance, and remaining concerns. The Implementation worker then sends its absolute path to the retained Owner pane ID:

```sh
bash <orc-impl-dir>/scripts/herdr-worker.sh send \
  --wait-ready \
  --pane <owner-pane-id> \
  --message "IMPL_COMPLETE: whole spec implemented and verified. Implementation report: <absolute-markdown-path>"
```
...
1. Reads the Implementation report and verifies the repository state against the pinned starting `HEAD` and every spec acceptance criterion.
2. Invokes `code-review` against that fixed point and the complete spec. Review the whole integrated diff only after the Implementation report is available.
3. Triages review findings. Send deterministic fixes back to the same Implementation worker as one consolidated batch using `send --wait-ready`. End the Owner turn again only after the helper confirms `accepted: true` and `status: working`, then wait for another explicitly accepted Completion callback.
   If the user adds an in-scope requirement while that fix turn is already `working`, deliver it with `steer` and require the next Completion callback to account for it.
...
The Owner owns triage, user decisions, final acceptance, and the Owner's final report. The Implementation worker owns implementation and deterministic review fixes.
...
At completion, the Owner's final report records the starting commit, Implementation worker pane/tab, ticket and commit coverage, verification evidence, review iterations, judgement calls, changed-LOC variance, and remaining blockers.
