...
3. **Branch: is this a multi-session build?**
   ...
   For a single-session build, **`/implement`** drives **`/tdd`** internally (one red-green slice at a time), then runs **`/code-review`** before committing. For a multi-session build, **`/orc-impl`** owns the complete implementation and whole-spec review. Reach for **`/tdd`** on its own when you want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a diff against a fixed point.
