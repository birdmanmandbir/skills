...
### Existing local Axiom CLI

If `scripts/init` has no Axiom deployment but the local `axiom` CLI can run
`axiom dataset list` non-interactively, use that CLI's existing authenticated
profile. Do not read, copy, or add its token to `~/.config/axiom-sre`.

In this mode, `axiom dataset list` is the required dataset discovery step;
then use `axiom query` with an explicit time range and run `getschema` before
filtering a dataset. Do not invoke the API-based `discover-axiom` or
`axiom-query` scripts, because those intentionally require a separately
configured API token.

- **DO NOT GUESS** dataset names like `['logs']`. You don't know them until you run `scripts/discover-axiom` or, in local CLI mode, `axiom dataset list`.
